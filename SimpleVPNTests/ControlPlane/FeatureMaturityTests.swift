...
struct DiagnosticReportSeamTests {
    ...
    @Test func aRegisteredPresenterReceivesEveryRequest() {
        let coordinator = DiagnosticReportCoordinator()
        let spy = Spy()
        coordinator.presenter = spy
        let request = DiagnosticReportRequest(kind: .fortinet, profileID: "abc",
                                             reason: .untestedKind)
        coordinator.presentReport(request)
        #expect(spy.seen == [request])
        #expect(coordinator.lastRequest == request)
    }

    /// With nothing installed, the placeholder answers — a banner's report button
    /// is never dead.
    @Test func withoutAPresenterThePlaceholderAnswers() {
        let coordinator = DiagnosticReportCoordinator()
        var opened: [URL] = []
        PlaceholderIssueTrackerPresenter.shared.open = { opened.append($0) }
        defer { PlaceholderIssueTrackerPresenter.shared.open = { _ in } }
        coordinator.presentReport(.init(kind: .ikev2, profileID: nil, reason: .untestedKind))
        #expect(opened.count == 1)
        #expect(opened.first?.absoluteString.hasPrefix(Acknowledgements.sourceURL) == true)
    }

    /// The placeholder's URL: this project's own tracker, the bug-report form, and a
    /// body that names the kind and asks the one question only the user can answer.
    @Test func thePlaceholderPrefillsThisProjectsIssueForm() throws {
        let url = try #require(PlaceholderIssueTrackerPresenter.issueURL(
            for: .init(kind: .globalProtect, profileID: "p1", reason: .untestedKind)))
        #expect(url.absoluteString.hasPrefix("\(Acknowledgements.sourceURL)/issues/new?"))
        #expect(url.absoluteString.contains("template=bug_report.yml"))
        let body = PlaceholderIssueTrackerPresenter.body(
            for: .init(kind: .globalProtect, profileID: "p1", reason: .untestedKind))
        #expect(body.contains("Palo Alto GlobalProtect"))
        #expect(body.contains("untested"))
        // Prompted, not a blank box.
        #expect(body.contains("?"))
    }

    /// Every reason produces a body, and none of them carries the profile id — the
    /// request may hold one, the prefilled URL never does.
    @Test func noReasonLeaksTheProfileIDIntoTheURL() throws {
        for reason in [DiagnosticReportRequest.Reason.untestedKind, .untestedSource,
                       .connectFailure, .userInitiated] {
            let request = DiagnosticReportRequest(kind: .pulse, profileID: "SECRETPROFILEID",
                                                 reason: reason)
            let url = try #require(PlaceholderIssueTrackerPresenter.issueURL(for: request))
            #expect(!url.absoluteString.contains("SECRETPROFILEID"))
            #expect(!PlaceholderIssueTrackerPresenter.body(for: request).isEmpty)
        }
    }

    /// Reasons are a closed, stable set — B0c's real sheet keys off them.
