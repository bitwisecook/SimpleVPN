...
6. **Custom-drawn surfaces are navigable structures, not labeled pictures.** Canvas/graph
   views expose children and rotors (Routes window: "VPNs", "Guests" and "Problems"
   rotors, one element per card/edge/drift line, custom actions matching clicks). The
   guest-network cards are the reference for a card that HOLDS a control: `.contain`
   plus a container label (rule 4), the choice's consequence as both `.help` and
   `accessibilityHint`, and — where there is no choice — the reason in the card's own
   value rather than a dead switch (rule 5). Swift Charts get
...
9. **The regression gate is real, and it is two gates.** SimpleVPNUITests runs
   `performAccessibilityAudit()` over the **main, Routes, Settings (all three panes —
   General, Sign-In Sources, Labels — driven in turn, not whichever tab was last used),
   Manage VPNs, Network Tools and Report a Problem** windows with one shared exclusion
   list (SimpleVPNUITests.swift documents each exclusion and why it is a framework
   artifact) — the audit answers "is anything unnamed or unreachable?".
   `VoiceOverWalkthroughTests` answers the different question "does what VoiceOver would
   SAY match what this document promises?", step by step (see "The walkthrough,
   automated" below). New failures in either are build-breaking, same as warnings.

   **Still unaudited, and known to be:** SimpleVPN Help (the manual window), About
   SimpleVPN, the VPN Sign-In (SSO) window, and every sheet — Find a Setting, Discover a
   VPN, the composition editor, Map 1Password Fields, the crash/issue/UI report sheets
   and the traffic log. Listed rather than left implicit, because "the audit is green"
   must not be read as "everything was audited".

   **One exclusion earns its own warning.** The shape-based excusal for an anonymous,
   unlabelled `.group` cannot distinguish NSHostingView/NavigationSplitView chrome from
   an app-owned `.accessibilityElement(children: .contain)` that simply forgot its
   container label — the two are byte-identical to the audit. It was silently excusing
   three real surfaces (`FirstConnectSetupCard`, `OnePasswordSetupCard`'s walkthrough and
   SignInSourcesSettings' "where it was found" disclosure) until they were labelled. A
   `.contain` container without an `accessibilityLabel` is therefore a defect the gate
   will NOT catch for you: rule 4 has to be honoured by hand.
...
| Which vault, then which entry (`SignInInstanceEntryPicker`, in the VPN editor's Sign-In tab and under the chooser's chosen row) | TWO NUMBERED STEPS, spoken as such. Each step's heading is one `.combine`d element with the `.isHeader` trait, so VoiceOver can move step to step, and its label is the question plus the line saying what belongs in it. Step 1's picker: label = "Step 1 of 2: which database", value = `SignInSourceSteps.spokenStep` ("Step 1 of 2. Work." / "…No database chosen yet."), hint = what choosing does and where they are set up. Step 2's heading NAMES the vault chosen in step 1 ("Step 2 of 2: which entry in “Work”"), so the pair reads as one sentence rather than two unrelated fields; its field's title argument is empty and the example rides `prompt:`. The chosen vault's own live state is a `Label` whose text carries it (never colour alone) and whose value is the same sentence — including "the database this VPN used isn't set up any more", which is said HERE rather than discovered as a connect failure. A vendor that declares itself single-instance shows step 2 alone, unnumbered: a one-item picker is a question with no answer. |
| Maturity banner + badge (`MaturityBanner`, `MaturityBadge`) | "Nobody has proven this yet", and it is CONTENT. The banner is a `.contain` container whose own label is the whole notice plus the line saying what each button does (`MaturityNotice.spokenSummary` + `NoticeActionsCaption.sentence`) — never a colour, never a hover: the report button and the collapse button stay reachable inside, each naming what it does and what stays behind ("The “Untested” label stays either way"). **What clicking does is ON SCREEN, not only in a tooltip** — a reported confusion ("it wasn't clear, i clicked on it") whose answer lived entirely in `.help` and a hint. The visible sentence and the spoken one are the same string, and the buttons are named in it ("Hide Details", "Show Details", "Tell Us What Happened") rather than a bare "Hide" or an ellipsis that promises a dialog. Collapsing is announced, and the collapsed form still SPEAKS the state, because it still shows the badge. In list rows (main sidebar, Manage VPNs, menu bar) the chip is `accessibilityHidden` and its words ride the row's own sentence instead — the same rule as the status dots — appended last, after name, kind, state and labels, because it is the least urgent and most permanent thing about a row. On a sign-in row the chip sits beside the title and the maturity is a SECOND clause in the row's `accessibilityValue` after its live state ("Ready to use. Untested — nobody has confirmed KeePassXC working yet"): availability and maturity are different facts and are never merged into one phrase. Every word comes from the registry (`FeatureMaturity.swift`), so what VoiceOver says about a kind changes on the same one line the visible banner does. |
...
| Status banners (`ConnectionBanners.swift`) | Each banner is a `.contain` CONTAINER whose own `accessibilityLabel` is every sentence it shows, in order — never a `.combine`, because each of them holds a button and a banner-wide combine swallows it (rule 4; this file was the last place that class of bug survived, and in the captive-portal and paused banners the swallowed button was the only way out). Every button also names its own consequence rather than repeating a bare verb: "Stop trying to reach Tig Lab", "Resume — put traffic back through the VPN", "Forget that Tig Lab couldn't be reached from Home Wi-Fi". Only `ConnectedBanner`/`ConnectedChip`/`ProblemPill` stay `.combine` — they hold no controls. |
| Guest-network capture banner (`GuestNetworkCaptureBanner`) | "This VPN will cut off Apple Containers" — a `.contain` container whose sentence is the headline, what is running, that SimpleVPN has changed nothing, and the CONSEQUENCE verbatim from `VirtualizationBypassOffer.consequence` (the split-tunnel trade-off is spoken, never implied by an amber tint). "Keep It Reachable" and "Not Now" stay reachable inside, each naming what it will do — the accept button names the subnets and the reconnect in its `.help`. Silent unless a guest network is live AND this tunnel would capture it, so it never becomes a banner people learn to skip past. |
| Connection telemetry / interface traffic | `AXChartDescriptor` audio graphs, combined stat rows. |
| Routes window | Named container "Route diagram"; one element per card with label+value+actions; "VPNs"/"Guests"/"Problems" rotors; drift lines and guest networks are rotor entries; edges: dash + symbol + words, DWC dash rhythms; keyboard pan/zoom. A guest card's label names the product, the arrangement and the subnet; its value carries **every** guest name (not the card's truncated two — VoiceOver has no width limit), where their traffic goes now, and why the choice is unavailable when it is. The inspector's roster rows combine into one sentence each: name, product, one of the three presence states in `ONTOLOGY.md`'s words, and the evidence for the placement. |
| Interface traffic chart | `AXChartDescriptor` audio graph, one download and one upload series per plotted connection. A guest's series is named "<name>, with this Mac" and the summary repeats the caveat, because a VoiceOver user hearing "postgres, download" has no caption in earshot and would otherwise draw the conclusion the caption exists to prevent: how much of a guest's traffic went through a VPN is not measurable and is never encoded as a number. |
| Gateway bar | Value = owner in words; announcer speaks gateway changes. |
| Manage VPNs | Combined rows for every store (NE, tunnels, native, compositions) incl. status words; toolbar +/− and "Find a setting"; **the list is reorderable and the drag is never the only way** — a Tab-reachable Move Up / Move Down pair beside +/− (no key equivalent here: the servers table in this same window already owns ⌘⌥↑/⌘⌥↓, and two pairs in one window make it ambiguous), the same two commands on every row's context menu, and the platform `List` reorder, all three going through one `ReorderCommands` so the refusals and the announcement cannot fork. The order is ONE order, shared with the main window's sidebar (`ConnectOrder`), a move changes no selection tag, and a move cannot take a row out of its group — a group says what connecting a VPN does to this Mac, which a gesture may not change; **export is a per-row context-menu action, offered only for a kind that has a format** (`.ovpn` for OpenVPN, `.conf` × 2 for WireGuard, nothing for the rest) — it was a toolbar button acting on the window, which said nothing about WHICH VPN it meant; composition row menu; sidebar initial focus. |
| Add servers from dropped configuration files (`AddServersFromFilesSheet`) | **The drag is never the only way, and the sheet is a structure rather than a picture.** Three surfaces accept the drop — a VPN's row in either sidebar and the Servers table — and every one of them has a named equivalent that needs no pointer: "Add Servers from Configuration Files…" on both sidebars' row menus and in Manage VPNs' `+` menu, plus a Tab-reachable button beside `+` / `−` in the Servers table whose disabled state carries its reason in `.help` AND `accessibilityValue`. The Servers table footer states in words that the drop exists. In the sheet, ONE ELEMENT PER FILE: label = the file's name, value = the whole sentence saying what that file will do, which is the same sentence shown beside it — a count alone would not say which of six files was refused. The glyphs are `accessibilityHidden`; their meaning rides the sentence, the same rule the status dots follow. The header combines into one `.isHeader` element carrying the counts, and the summary is announced with `sayNow` when the sheet opens because the user just made a gesture and is waiting for the answer. Cancel owns Escape and nothing carries `.defaultAction`. Every string is in `ConfigurationDropCopy`, test-enforced against ONTOLOGY.md — "credential" and "endpoint" among the banned words. |
| Accepting a changed provider server list (`ProviderListUpdateSheet`) | **A held update, said per server and led by the dangerous one.** The rows are grouped under headings in RANK order, not alphabetically: a changed public key on a server the user holds first, then one on a server they do not, then a changed address, then servers no longer listed, then arrivals — because for WireGuard the peer key IS the authentication and there is no certificate behind it. Each row is one element: label = the hostname, value = what changed in words ("Its public key changed. You have this server."), never a colour or a glyph alone. Above the list, the two warnings are `.contain`-free `Label`s whose own combined element carries the whole sentence — what a public key is *for* before what changed, and why losing a third of a list holds the update. There is not one tick box: the sheet states the all-or-nothing rule in words where somebody would look for one. The SAFE button is the prominent one and owns Escape; the accepting button carries no key equivalent at all, so Return cannot accept a moved key. Opening announces the title, the counts and "Nothing has been changed" together. Every string is in `ProviderListUpdateCopy` and pinned by test. |
...
| Settings ▸ Sign-In Sources (`SignInSourcesSettings`) | Every row is an `EngineSettingRow` over a `creds.*` spec, so the label, summary, manual link and reveal come from the registry rather than from the view. **The value/suggestion distinction is carried in WORDS**: a path field's title argument is always empty (the name is the spec's), a detected path is a `prompt:` placeholder plus its own "SimpleVPN found" row, and the field's `accessibilityValue` says which of the two you are looking at ("Not set. SimpleVPN uses the one it found: …" versus the path followed by its validation) — grey-versus-black reaches nobody using VoiceOver. Validation state is part of that spoken value AND a visible `Label` that announces its role ("Problem: …"). "Use What SimpleVPN Found" / "Clear" each carry their reason when disabled. The "where it was found" disclosure lists every path with, in words, whether SimpleVPN will run it from there. A row locked by MDM says whose decision it is. **The KeePass database rows add three things no other vendor needs**: the database password is a `SecureField` whose spoken value is never the secret and never its length but WHAT SIMPLEVPN IS HOLDING ("Not held" / "Held until SimpleVPN quits" / "Remembered behind Touch ID"), and that same sentence is a visible row beside it and is announced when it changes; the "Remember with Touch ID" toggle says why it is unavailable on a Mac that cannot ask for a fingerprint; and "Check My Security Key" announces "Touch your security key now" when armed, carries "waiting for you to touch your security key" as its value while waiting, and announces its whole result sentence — a check whose outcome is only a coloured icon would be invisible. The file fields additionally offer a "Choose Database…" / "Choose Key File…" button ALONGSIDE the text field, never instead of it: a typed path must always work, because that is what MDM pins and what somebody reading over a shoulder follows. **The list of vaults** (a vendor that can have several — today the KeePass database row) is one selectable element per vault: label = its name, value = ITS OWN state sentence, hint = that picking it shows that vault's file, key file and slot below; the chosen one carries `.isSelected` and picking it is announced with name and state together. So "the work database is missing and the personal one is ready" is two spoken sentences rather than one averaged answer. Add / Rename / Remove each carry their reason when unavailable, and Remove's confirmation NAMES the VPNs that read that vault before anything is removed — an orphaned VPN would otherwise fail at connect time with nothing pointing back at this pane. |
| About | Identity block combined. Component rows are `.contain` CONTAINERS whose own sentence is "name, role, licence X" — rule 4, because each row holds a link to that project and a row-wide `.combine` swallows it; the role and licence `Text`s are `accessibilityHidden` so they ride the sentence instead of being read twice, leaving the link as the one reachable child. |
| Diagnostics sheets (crash, issue, error) | ESC + default action; remedy steps read without markup. |
| Report a Problem (`DiagnosticReportView`) | Two questions, each a `Text` heading plus a `Text` example ABOVE its box — the example is never the field's title, so VoiceOver reads "What you were doing", not the worked example (rule 5's sibling: the landmine that produced 26 fixes). Initial focus lands in the first box. Each per-section switch is its own element: label = the section's title, value = "included"/"left out", hint = the same "why this helps" sentence the caption shows — so the reason is content, not a caption sighted users get for free; the visible caption is `accessibilityHidden` so it isn't said twice. Choosing one is announced. The payload preview is a named, editable text view ("The report that will be shared") whose hint says it is the exact text that leaves the Mac. The primary button carries its reason when disabled in `.help` AND `accessibilityValue`. Every outcome ("Report copied to the clipboard") is a visible label AND an announcement — nothing about this flow happens silently. ESC closes the window (`cancelOperation` on its own `NSWindow` subclass; a plain `NSWindow` does not do this for free). |
...
| 6 | **Use the rotor** (VO-U → "VPNs", then "Guests", then "Problems") | The diagram is a named container ("Route diagram") with children; every button inside is named; at least one card carries both a name and a value | The rotors themselves: that "VPNs", "Guests" and "Problems" are offered, that a healthy connection lists no problems, and — with a container running — that a guest card reads as one sentence with its choice reachable inside it |
...
| 9b | **Rearrange the VPN list** — Tab to the Move Up / Move Down pair beside **+** and **−**, move a VPN, then right-click a row and do it from the menu | The order is one order (both windows read the same arrangement); a move changes no selection tag; a move cannot take a row out of its group; the ends of a group refuse with a reason that NAMES the group ("… is already last in Whole-Mac VPNs"); the announcement names what moved, the position it now holds and the group it holds it in | The parts no unit test reaches, all of them pointer-shaped: that the drag starts at all, that AppKit's insertion line appears between the right two rows, that dropping into the OTHER group is refused on screen and not merely undone afterwards, and that what follows the pointer is the platform's static row snapshot rather than a live Connect button. Also: that the pair has no ⌘⌥↑/⌘⌥↓ here **on purpose** — the servers table in the same window owns that shortcut — and that Tab still reaches it |
